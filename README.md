...
To call `/dg_logout` and `/dg_info`, just issue a request on an URL similar to the `redirectURL` parameter by replacing `dg_callback` by `dg_logout` or `dg_info`. For example: `https://apache.ingress.mycluster.mycompany.com/dg_logout`
...
Only one of these configuration must be used.
...
Dexgate is in fact mainly the integration of pre-existing components. Mostly base on the [Dex example application](https://github.com/dexidp/dex/tree/master/examples/example-app), it also uses : 
